...
export function useFichadas() {
   ...
   */
  async function processFichadas({ filePath, fechaDesde, fechaHasta, legajos, feriados }) {
    const authHeaders = await getAuthHeaders();
    ...
    const response = await fetch(`${baseUrl}/api/fichadas/process`, {
      ...
      body: JSON.stringify({ filePath, fechaDesde, fechaHasta, legajos, feriados }),
    });
   ...
   */
  async function downloadExcel({ filePath, fechaDesde, fechaHasta, legajos, feriados }) {
    const authHeaders = await getAuthHeaders();
