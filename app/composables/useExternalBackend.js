...
export function useExternalBackend() {
    ...
    const baseUrl = config.public.externalBackendUrl || "http://localhost:4000";
    ...
    const request = async (endpoint, options = {}, withAuth = false) => {
      ...
      if (!response.ok) {
        throw new Error(
          data.error?.description || data.message || `HTTP ${response.status}: ${response.statusText}`
        );
      }
    ...
    const sourceListFiles = async (sourceName) => {
      ...
      return await request(`/api/sources/${sourceName}/files`, {
        method: "GET",
      }, true);
    };
    ...
    const sourceDeleteFile = async (sourceName, filename) => {
      ...
      return await request(`/api/sources/${sourceName}/files/${encodeURIComponent(filename)}`, {
        method: "DELETE",
      }, true);
    };
